...
ALTER TABLE employees ADD new_salary VARCHAR (30) null
...
select first_name from employees order by ASC;
...
SELECT TOP(1)  SALARY FROM (SELECT DISTINCT TOP (2) SALARY FROM EMPLOYEES ORDER BY SALARY DESC) AS EMPLOYEES ORDER BY SALARY;
...
SELECT * FROM EMPLOYEES WHERE HIRE_DATE BETWEEN '01-JAN-1999' AND '31-DEC-1999';
...
SELECT *  FROM EMPLOYEES WHERE SALARY <=9000;
 ...
 ALTER TABLE COURSES ADD  FEE DECIMAL(8,2);
 ...
 ALTER TABLE COURSES DROP COLUMN CREDIT_HOURS,MAX_LIMIT;
...
SELECT * FROM  EMPLOYEES WHERE SALARY >( SELECT AVG(SALARY) FROM EMPLOYEES WHERE 
 ...
 SELECT *  FROM EMPLOYEES WHERE hire_date Between '01-jan-1999' and '31-dec-2000';
 SELECT *  FROM EMPLOYEES WHERE hire_date NOT Between '01-jan-1989'and '31-dec-1999';
...
SELECT * FROM Employees INNER JOIN Departments ON Employees.department_id = Departments.department_id WHERE Employees.department_id IN (1, 2, 3);
select Employees.first_name, Employees.last_name FROM EMPLOYEES INNER JOIN departments ON Employees.department_id = Departments.department_id WHERE Employees.department_id IN (1, 2, 3);
...
select countries.country_name From countries left join locations on countries.country_id=locations.country_id where locations.country_id IN ('US','UK','CHINA');
SELECT LOCATIONS.location_id FROM LOCATIONS LEFT JOIN COUNTRIES ON countries.country_id=locations.country_id WHERE locations.country_id IN ('US','UK','CHINA');
SELECT COUNTRIES.COUNTRY_ID,LOCATIONS.location_id FROM LOCATIONS LEFT JOIN COUNTRIES ON countries.country_id=locations.country_id;

SELECT * FROM REGIONS;

SELECT COUNTRIES.COUNTRY_ID,LOCATIONS.location_id,REGIONS.REGION_ID
FROM LOCATIONS 
...
INNER JOIN REGIONS ON COUNTRIES.REGION_ID=REGIONS.REGION_ID;
 ...
 select * from employees e join employees e1 on e1.manager_id=e.employee_id ;
...
SELECT
    ...
    job_id,
    COUNT(*) as RowCount,
    GROUP_CONCAT(employee_name) as EmployeeNames
FROM Employees
...
FROM Employees
GROUP BY department_id
HAVING LowestSalary > 10000;


SELECT department_id, AVG(Salary) AS AverageSalary
FROM Employees
GROUP BY department_id
HAVING AverageSalary BETWEEN 5000 AND 7000;
...
select employee_id  from employees union all select employee_id from dependents;
...
select employee_id from employees intersect select employee_id from dependents order by employee_Id desc;
...
case
when (hire_date >2000 then '1 year')
...
SELECT
    ...
    CASE
        ...
        ELSE 'Unknown'  -- This handles any unexpected cases
...
SELECT AVG(AverageSalary) as OverallAverageSalary
FROM (SELECT department_id, AVG(Salary) as AverageSalary
      FROM Employees
      GROUP BY department_id) AS DepartmentAverages;
